function [total_boxes1, total_boxes2] = centroid_parallel_detector(gt_path, image_input, inpath_det1, inpath_det2, annotation_path, output_impath, min_overlap)
...
% sizes of box are mean of width and height of other boxes
    ...
    % detect everything, remove difficult when evaluating
%         if ~isempty(find(gt_struct(i).Difficult==1))
%             continue
%         end
        ...
        matched_idx = []; % indeces of matched boxes in det2
        m = [];
        match = false;
        for j1=1:size(bboxes_1,1) % loop through det 1
            ...
            for j2=1:size(bboxes_2,1)
                ...
                if ~isempty(find(matched_idx==j2))
                    match = false;
                end
                if match
                    bb2 = bboxes_2(j2,:);
