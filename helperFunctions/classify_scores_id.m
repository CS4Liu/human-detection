...
function [true_positives, false_positives, misdetections, true_positive_scores, false_positive_scores] = classify_scores_id(gt_path, det_path, min_overlap, include_difficult)
    
    load(gt_path);
    load(det_path);
    
    % results = results_hogt0_dpmt1_dpmscore;
    ...
    true_positive_scores = struct('Scores',[], 'File_ID', []);
    false_positive_scores = struct('Scores',[], 'File_ID', []);

    image_ids = size(results_dummy);
    image_ids = image_ids(1);
    
    true_pos_scores_id_count = 1;
    false_pos_scores_id_count = 1;

    nd_ids = get_non_difficult_indexes(gt_bboxes_dummy);
    for z=1:length(nd_ids)
        i = nd_ids(z);
        this_result = results_dummy(i);
        % this_result_id = this_result.File_ID;
        ...
        for j=1:sd(1)
            ...
            for k=1:s(1)
                ...
                if ov>=min_overlap
                    ...
                    true_positive_scores(true_pos_scores_id_count).Scores = this_result.Scores(j);
                    true_positive_scores(true_pos_scores_id_count).File_ID = this_result.File_ID;
                    true_pos_scores_id_count = true_pos_scores_id_count+1;
        ...
        if ~isempty(this_result_false_scores)
            for res = 1:size(this_result_false_scores,1)
                false_positive_scores(false_pos_scores_id_count).Scores = this_result_false_scores(res);
                false_positive_scores(false_pos_scores_id_count).File_ID = this_result.File_ID;
                false_pos_scores_id_count = false_pos_scores_id_count + 1;
            end
        end
